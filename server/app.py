...
class RestaurantHandler(BaseHTTPRequestHandler):
    ...
    def record_audit(self, db, action, target_type="", target_id="", detail=""):
        admin = getattr(self, "current_user", None) or {}
        db.execute(
            """
            INSERT INTO admin_audit_log
                (admin_user_id, admin_username, action, target_type, target_id, detail, ip, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                admin.get("id"),
    ...
    def update_settings(self):
        ...
        with connect_db() as db:
            ...
            self.record_audit(db, "settings.update", "settings", "1")
            settings = db.execute("SELECT name, address, phone, hours, updated_at FROM restaurant_settings WHERE id = 1").fetchone()
    ...
    def create_menu_item(self):
        ...
        with connect_db() as db:
            ...
            self.record_audit(db, "menu.create", "menu_item", row_to_dict(item)["id"], name)
    ...
    def update_menu_item(self, item_id):
        ...
        with connect_db() as db:
            ...
            if item:
                self.record_audit(db, "menu.update", "menu_item", item_id)
    ...
    def delete_menu_item(self, item_id):
        with connect_db() as db:
            ...
            if deleted:
                self.record_audit(db, "menu.delete", "menu_item", item_id)
    ...
    def create_table(self):
        ...
        try:
            with connect_db() as db:
                ...
                self.record_audit(db, "table.create", "restaurant_table", row_to_dict(table)["id"], table_number)
        except Exception as error:
            if not is_integrity_error(error):
                raise
    ...
    def update_table(self, table_id):
        ...
        try:
            with connect_db() as db:
                ...
                if table:
                    self.record_audit(db, "table.update", "restaurant_table", table_id)
        except Exception as error:
            if not is_integrity_error(error):
                raise
            self.json_response({"error": "Nomor meja sudah ada"}, 409)
    ...
    def delete_table(self, table_id):
        with connect_db() as db:
            ...
            deleted = cursor.rowcount > 0
            if deleted:
                self.record_audit(db, "table.delete", "restaurant_table", table_id)
        self.json_response({"deleted": deleted})
    ...
    def list_audit_log(self, query):
        ...
        with connect_db() as db:
            rows = db.execute(
                """
                SELECT id, admin_user_id, admin_username, action, target_type, target_id, detail, ip, created_at
        ...
        self.json_response({"entries": [row_to_dict(row) for row in rows]})
...
def self_test():
    ...
    with connect_db() as db:
        ...
        audit_columns = table_columns(db, "admin_audit_log")
    ...
    assert "idx_admin_audit_log_created_at" in indexes
